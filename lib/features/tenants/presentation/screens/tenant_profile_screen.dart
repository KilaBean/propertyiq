...
class TenantProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // An unclaimed invite has no profile to fetch yet, so that case resolves
    // immediately rather than going through the provider.
    if (t.tenantId == null) return _scaffold(context, ref, null);

    return AsyncValueView(
      value: ref.watch(profileByIdProvider(t.tenantId!)),
      onRetry: () => ref.invalidate(profileByIdProvider(t.tenantId!)),
      data: (profile) => _scaffold(context, ref, profile),
    );
  }

  Widget _scaffold(BuildContext context, WidgetRef ref, Profile? profile) {
    final t = args.tenancy;
